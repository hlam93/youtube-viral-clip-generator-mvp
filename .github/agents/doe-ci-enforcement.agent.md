...
- extend the vendored DOE governance automation for this product repository's stack: `scripts/validate_doe_artifacts.rb` (artifact shape, cross-file referential integrity, risk-tier reconciliation, DoD completeness) and the installed `.github/workflows/doe-governance.yml`, plus `scripts/check_commit_messages.sh` / `scripts/check_pr_title.sh`; add your product's lint/test/build/security jobs alongside them (CONSUMERS.md §5 has a recommended baseline checklist)
...
- Product-stack CI (lint/test/build/deploy) belongs in this product repository; the DOE framework's own self-governance scripts (for example `check_governance_consistency.sh`, `check_workspace_boundary.sh`) are maintainer-only and were not vendored here (CONSUMERS.md) — do not reference or try to extend them.
...
8. For this repository's own workflow/policy assets under `.github/workflows/`, keep them consistent with the vendored `SYSTEM.md` and detect drift early.
